...
test("testUrl() fetch HTML", async () => {
  ...
  const testUrl = await tests
    .testUrl("sodiumhalogen.com")
    .then(thenFn)
    .catch(catchFn);

  expect(mockAxios.get).toHaveBeenCalledTimes(1);
  expect(mockAxios.get).toHaveBeenCalledWith("sodiumhalogen.com");
...
test("checkForSitemap() checks sitemap url", async () => {
  ...
  const testSitemap = await tests
    .checkForSitemap("sodiumhalogen.com/sitemap.xml")
    .then(() => true);
  expect(mockAxios.get).toHaveBeenCalledTimes(1);
  expect(mockAxios.get).toHaveBeenCalledWith("sodiumhalogen.com/sitemap.xml");
  expect(testSitemap).toBeTruthy();

  mockAxios.get.mockImplementationOnce(() => Promise.resolve(false));
  const testBadSitemap = await tests
    .checkForSitemap("sodiumhalogen.com/sitemap.xmllll")
    .then(() => false);
...
test("checkLinks() checks for 404s", async () => {
  ...
  expect(mockAxios.get).toHaveBeenCalledWith("sodiumhalogen.com");
  expect(mockAxios.get).toHaveBeenCalledWith("alygn.com");
});
